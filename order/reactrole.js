    ...
    if (message.member?.permissionsIn(message.channel).has("ADMINISTRATOR")) {
      try {
        if (inputs[0] === "add") {
          ...
          message.client.log(`**REACT ROLE ADDED:** ${target.url || message.reference.messageId} (${inputs[1]} for ${inputs[2]})`)
          message.react("✅")
          ...
          if (inputs[1] == "all") {
            ...
            await message.client.log(`**REACT ROLES REMOVED:** ${reactMessage.url || target.message} (${reacts.join("")})`)
          } else {
            for (const reaction of userReactions.values()) {
              if (reaction.emoji.toString() == inputs[1]) {
                ...
                await message.client.log(`**REACT ROLE REMOVED:** ${reactMessage.url || target.message} (${inputs[1]})`)
              }
            }
          }
          await db.reactroles.reload()
